# [2026-07-07] 사람인 디딤(주) 신규 공고 자동 수집 배치

## 1. 작업 배경

수동으로는 이미 다음이 가능하다: 공고 URL 입력 → URL 상세 추출 → LLM 으로 JD 구조화 → 공고/JD 저장 → JD 저장 성공 시 Drive 공고 폴더 생성.
이번 작업은 이 수동 흐름을 **사람인 '디딤' 검색 결과 기반으로 주기 자동화**하여, 디딤(주)의 신규 공고를 자동으로 `job_postings`/`job_posting_jds` 에 등록하는 배치 구조를 추가한다.

## 2. 구현 목표

- 사람인 '디딤' 검색 URL 을 1시간마다 확인하는 배치 구조 추가(실제 스케줄 등록은 주석 처리).
...
- `app/services/scheduler_service.py` — 배치 진입점 `run_saramin_didim_discovery()` + 1시간 interval 등록(주석 처리).
- `docs/work-log/2026-07-07-saramin-job-discovery-batch.md` — 본 문서.

### 수정 (기존 로직 불변, 추가만)
- `app/api/jobs_router.py` — `POST /api/jobs/discover/saramin/didim` 수동 실행 엔드포인트 추가(기존 `extract-from-url` 불변).
- `app/core/config.py` — `SARAMIN_DIDIM_SEARCH_URL`/`_KEYWORD`/`_COMPANY_NAME`/`SARAMIN_DISCOVERY_ENABLED` 설정 추가.
...
→ 회사명 normalize(공백 제거) 후 exact 필터: 디딤(주)/디딤 (주)/디딤 주식회사 만 통과
...
→ 신규만: job_extract_service.extract_from_url(detail_url) [기존 로직 재사용: 디딤(주) 재검증 + JD 구조화]
...
```json
{
  "source": "SARAMIN", "keyword": "디딤", "company_filter": "디딤(주)",
...
```
- 스케줄러(무인) 실행은 `scheduler_service.run_saramin_didim_discovery()` 가 활성 ADMIN 계정 권한으로 같은 service 를 호출한다(수동/배치 동일 경로).

## 7. 검증 결과

일회용 Postgres 컨테이너(`postgres:16-alpine`) + 외부 호출(사람인 fetch/OpenAI/Google Drive) stub 으로 검증. `DATABASE_URL` 은 환경변수로만 주입(운영 `.env` 미변경).

- **단위(순수 함수)**: 회사명 normalize/필터 — 디딤(주)/디딤 (주)/디딤 주식회사 True, `디딤 정신건강의학과의원`·`(주)디딤 커뮤니케이션`·`메가디딤(주)` 등 False. detail_url 절대경로/추적 query 제거. 검색결과 HTML 파싱(디딤 계열만 매칭).
